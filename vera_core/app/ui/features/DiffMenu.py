...
def register_diff_state_ctrl(state, ctrl, registry: VeraDataRegistry):
    ...
    def create_diff_dataset():
        ...
        try:
            ...
            for k, v in (
                ("diff_name", ""), ("ref_dataset_name", ""), ("ref_src_id", None),
                ("ref_label", "Select dataset"), ("ref_units", ""), ("ref_scale", 1.0),
                ("comp_dataset_name", ""), ("comp_src_id", None),
                ("comp_label", "Select dataset"), ("comp_units", ""), ("comp_scale", 1.0),
                ("diff_units", ""), ("diff_error", ""), ("ref_shape", ""), ("comp_shape", ""),
            ):
                state[k] = v
    ...
    with vuetify.VDialog(v_model=("show_diff_dialog",), max_width=680, persistent=True):
        with vuetify.VCard():
            ...
            with vuetify.VCardText(classes="pt-4"):
                vuetify.VTextField(
                    v_model=("diff_name",), label="Name",
                    hide_details=True, dense=True, classes="mb-4",
                )
                ...
                with vuetify.VCard(outlined=True, classes="pa-3 mb-2"):
                    ...
                    with html.Div(classes="d-flex align-center", style="gap: 12px;"):
                        ...
                        vuetify.VTextField(
                            v_model=("ref_scale",), label="× scale", type="number",
                            hide_details=True, dense=True,
                            style="max-width: 110px; flex: 0 0 auto;",
                        )
                    html.Div("{{ 'Units: ' + ref_units }}",
                             classes="text-caption text--secondary mt-1", v_show=("ref_units",))
                    html.Div("{{ 'Shape: ' + ref_shape }}",
                             classes="text-caption text--secondary", v_show=("ref_shape",))
                ...
                with html.Div(classes="d-flex justify-center align-center my-1"):
                    vuetify.VSelect(
                        ...
                        hide_details=True, dense=True, solo=True, flat=True,
                ...
                with vuetify.VCard(outlined=True, classes="pa-3 mb-3"):
                    ...
                    with html.Div(classes="d-flex align-center", style="gap: 12px;"):
                        ...
                        vuetify.VTextField(
                            v_model=("comp_scale",), label="× scale", type="number",
                            hide_details=True, dense=True,
                            style="max-width: 110px; flex: 0 0 auto;",
                        )
                    html.Div("{{ 'Units: ' + comp_units }}",
                             classes="text-caption text--secondary mt-1", v_show=("comp_units",))
                    html.Div("{{ 'Shape: ' + comp_shape }}",
                             classes="text-caption text--secondary", v_show=("comp_shape",))

                # Shape / type incompatibility (blocks submission)
                vuetify.VAlert(
                    "{{ diff_shape_error }}", v_show=("diff_shape_error",),
                    type="warning", dense=True, text=True, classes="mb-3",
                )
                ...
                with html.Div(classes="d-flex align-center", style="gap: 16px;"):
                    vuetify.VSelect(
                        v_model=("diff_interp_kind",), items=("diff_interp_kinds",),
                        label="Interpolation", hide_details=True, dense=True,
                        style="max-width: 200px;",
                    )
                    vuetify.VTextField(
                        v_model=("diff_units",), label="Output units",
                        hide_details=True, dense=True,
                        placeholder="e.g. Celsius",
                    )

                vuetify.VAlert(
                    "{{ diff_error }}", v_show=("diff_error",),
                    type="error", dense=True, text=True, classes="mt-4 mb-0",
                )
            vuetify.VDivider()
            with vuetify.VCardActions(classes="px-4 py-3"):
                vuetify.VSpacer()
                vuetify.VBtn("Close", text=True, click="show_diff_dialog = false")
                vuetify.VBtn("Create", color="primary",
