...
def initialize(server, registry: VeraDataRegistry, view_id):
    ...
    def update_assembly_surface_view(**kwargs):
        if is_non_active_view(state, view_id, option):
            return
        _, _, selected_layer, selected_assembly, selected_src_id, selected_array = get_safe_idxs(
        ...
        for g in range(n_energy, MAX_VIS_GROUPS):
            state[group_keys[g]] = []
                    ...
                    with html.Div(
                        ...
                        with html.Div(style="flex: 1; min-height: 0; position: relative;"):
                            vera.AssemblySurfaceView(
                                ...
                                click=(on_surface_click, "[$event.i, $event.j, $event.surface]"),
                                dark=("dark_mode",),
                                busy=("trame__busy",),
                            )
            html.Div(
